...
jQuery.extend({
    ...
    dobPicker: function(params) {
        ...
        var defaultMonthValue = months.indexOf(params.monthDefault)+1;
        if(defaultMonthValue<10){
            defaultMonthValue='0'+defaultMonthValue;
        }
        ...
        $(params.monthSelector).append('<option value="' + defaultMonthValue + '">' + params.monthDefault + '</option>');
        $(params.yearSelector).append('<option value="' + params.yearDefault + '">' + params.yearDefault + '</option>');*/

        // -------------------------------------------------------

        // populate the day select
        $(params.daySelector).find('option').remove();
        $(params.monthSelector).find('option').remove();
        $(params.yearSelector).find('option').remove();
        ...
        for(i=1;i<=31;i++){
            ...
            if(val==params.dayDefault){
                $(params.daySelector).append('<option value="' + val + '" selected>' + val + '</option>');
            }
            else{
        ...
        $(params.daySelector).change(function(){
            ...
            if($(params.daySelector).val()>=1&&$(params.daySelector).val()<=29){
                ...
                var monthdis = [2,4,6,9,11];
                if(monthdis.indexOf(parseInt($(params.monthSelector).val()))!=-1){
                    $(params.monthSelector)[0].selectedIndex = 0;
                };
                $(params.monthSelector + ' option').removeAttr('disabled');
                $(params.monthSelector + ' option[value="02"]').attr('disabled', 'disabled');
                ...
                $(params.yearSelector + ' option').each(function(index){
                    if(index>=0){
                        ...
                        if(leap===false){
                            ...
                            if(i==0){
                                $(params.yearSelector)[0].selectedIndex = index;
                                ++i;
                            }
                        }
                    }
