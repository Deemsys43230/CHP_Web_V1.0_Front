...
commonApp.directive('myYoutube',['$sce', function($sce) {
    return {
        ...
        replace: true,
        template: '<div style="height:400px;"><iframe style="position: relative;z-index: 1;" width="100%" height="100%" src="{{url}}" frameborder="0" wmode="Opaque" allowfullscreen></iframe></div>',
        link: function (scope) {
...
userApp.controller('UserVideosController',['$scope','requestHandler','Flash',function($scope,requestHandler,Flash) {
...
// url filter
userApp.filter('trustUrl',['$sce', function ($sce) {
    return function(url) {
        return $sce.trustAsResourceUrl(url);
    };
}]);

userApp.directive('myYoutube',['$sce', function($sce) {
    return {
        restrict: 'EA',
        scope: { code:'=',
            autoplay:'='
